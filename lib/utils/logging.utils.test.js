...
const generateErrorColorLog = log => `\x1b[31m${log}\x1b[39m`;
const generateInfoColorLog = log => `\x1b[36m${log}\x1b[39m`;
const generateSuccessColorLog = log => `\x1b[32m${log}\x1b[39m`;
...
test('logMessage:error', t => {
    ...
    t.equal(onLogSpy, generateErrorColorLog('Error: error:test'), 'onLogSpy event was called with error message');
    t.end();
});
...
test('logMessage:info', t => {
    ...
    t.equal(onLogSpy, generateInfoColorLog('Info: info:test'), 'onLogSpy event was called with info message');
    t.end();
});
...
test('logMessage:success', t => {
    ...
    t.equal(onErrorSpy, false, 'onErrorSpy event was not called');
    t.equal(onLogSpy, generateSuccessColorLog('Success: success:test'), 'onLogSpy event was called with success message');
    t.end();
});